...
function evolve(population::Population,genN,mutateR,maxYears,oneOverP,groupN,trialC,filename)
    ...
    for genC in 1:genN
		...
		for groupC in 1:length(population.fitness)
	 	    for agentC in 1:length(population.groups[groupC].fitness)
		    fitValue=population.groups[groupC].fitness[agentC]
		    println(file,"$(genC) $(trialC) $(groupC) $(agentC) $(oneOverP)  $(fitValue)")
	   	 end
		 end
	end
	end
